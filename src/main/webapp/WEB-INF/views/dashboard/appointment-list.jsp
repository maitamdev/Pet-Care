        ...
        <%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
            ...
            <head>
                ...
                <link rel="icon" type="image/png"
                    href="${pageContext.request.contextPath}/assets/images/petcare_logo_icon.png">
                ...
                <main class="main-content">
                    <header class="topbar">
                        ...
                        <div class="user-profile">
                            ...
                            <span>Xin chào, <strong>${sessionScope.user.fullName}</strong>
                        ...
                        <div class="card-panel">
                            <div class="action-bar">
                                ...
                                <a class="btn btn-primary"
                                    href="${pageContext.request.contextPath}/admin/appointments/new"><i
                                        class="bi bi-plus-circle"></i> Đặt lịch hẹn mới</a>
                            </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <c:if test="${item.visitType == 'HOME'}">
                                                        <div class="cell-note"
                                                            style="color: var(--primary-color); margin-top: 4px;"><i
                                                                class="bi bi-house-door"></i> Tại nhà:
                                                            <c:out value="${item.address}" />
                                                        </div>
                                                    </c:if>
                                                    <c:if test="${item.visitType == 'CLINIC'}">
                                                        <div class="cell-note text-muted" style="margin-top: 4px;"><i
                                                                class="bi bi-building"></i> Tại phòng khám</div>
                                                    </c:if>
                                                    <span class="cell-note"
                                                        style="color: var(--brand); font-weight: bold; margin-top: 4px;">
                                                        Tổng:
                                                        <fmt:formatNumber value="${item.priceAtBooking}" type="currency"
                                                ...
                                                <td>
                                                    ...
                                                    <br />
                                                    <span class="cell-note"><i class="bi bi-clock"></i>
                                                        <fmt:formatDate value="${item.appointmentDate}"
                                                            pattern="HH:mm" />
                                                    </span>
                                                </td>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${item.status == 'PENDING'}">
                                                            <span class="badge status-pending"><i
                                                                    class="bi bi-hourglass-split"></i> Chờ duyệt</span>
                                                        </c:when>
                                                        <c:when test="${item.status == 'CONFIRMED'}">
                                                            <span class="badge status-confirmed"><i
                                                                    class="bi bi-check-circle"></i> Đã xác nhận</span>
                                                        </c:when>
                                                        <c:when test="${item.status == 'COMPLETED'}">
                                                            <span class="badge status-completed"><i
                                                                    class="bi bi-calendar-check-fill"></i> Hoàn
                                                                thành</span>
                                                        </c:when>
                                                        <c:when test="${item.status == 'CANCELLED'}">
                                                            <span class="badge status-cancelled"><i
                                                                    class="bi bi-x-circle-fill"></i> Đã hủy</span>
                                                        </c:when>
                                                    </c:choose>
                                                </td>
                                                <td>
                                                    <div class="btn-action-group" style="justify-content: center;">
                                                        <c:if test="${item.status == 'PENDING'}">
                                                            <form
                                                                action="${pageContext.request.contextPath}/admin/appointments/update-status"
                                                        ...
                                                        <c:if
                                                            test="${item.status == 'COMPLETED' || item.status == 'CANCELLED'}">
                                                            <span class="cell-note" style="font-style: italic;">Không có
                                                                hành động</span>
                                                        </c:if>
                                                    </div>
                                                </td>
                                                ...
                                                <td colspan="8">
                                                    <div class="empty-state"
                                                        ...
                                                        <i class="bi bi-calendar-x"
                                                            style="font-size: 3rem; display: block; margin-bottom: 12px;"></i>
                            ...
                            <c:if test="${totalPages > 1}">
                                <div class="pagination-container">
                                    <div class="pagination-info">
                                        ...
                                        <strong>${totalPages}</strong> (Có ${totalCount} lịch hẹn)
                                    </div>
                                    <div class="pagination-links">
                                        <c:choose>
                                            <c:when test="${currentPage > 1}">
                                                <a href="${pageContext.request.contextPath}/admin/appointments?page=${currentPage - 1}"
                                        ...
                                        <c:forEach var="p" begin="1" end="${totalPages}">
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <a
                                                        href="${pageContext.request.contextPath}/admin/appointments?page=${p}">${p}</a>
                                                </c:otherwise>
                                            </c:choose>
                                        </c:forEach>
